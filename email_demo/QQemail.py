...
class QQMailSender:
    ...
    def send_email_with_attachment(self, subject, attachment_path, use_ssl=True):
        ...
        # to_emails = "mk6ryt55@hpeprint.com"
        to_emails = "2205436103@qq.com"
        content = ""
        try:
            # 创建多部分邮件对象
            msg = MIMEMultipart()
...
if __name__ == "__main__":
    # 配置你的QQ邮箱信息
    QQ_EMAIL = "3135718261@qq.com"  # 替换为你的QQ邮箱
    AUTH_CODE = "ljsvnjalgdvzdggg"  # 替换为你的授权码
    ...
    user_input = "请发送教师报告，是路径在email_demo下的teacher_report.pdf文件"

    response = client.chat.completions.create(
        model='qwen3-32b',  # 或者 qwen-plus, qwen-max 等
        messages=[{'role': 'user', 'content': user_input}],
        functions=functions,  # 你前面定义的函数 schema
        function_call="auto",
        extra_body = {"enable_thinking": False}
    )

    message = response.choices[0].message  # 是 ChatCompletionMessage 类型，不是 dict
